...
def run_foldseek(input_dir:str, foldseek_DB:str, output_tsv:str, threads:int=mp.cpu_count(), foldseek_path:str=shutil.which('foldseek'), evalue:float=10, tmp_dir:str=None):
...
def read_tsv(out_tsv:str):
    ...
    with open(out_tsv, 'r') as f:
        for line in f:
            ...
            if len(split_line) != 8:
                continue
            query, target, evalue, bits, alntmscore, qtmscore, ttmscore, prob = split_line
...
def determine_sto(query_dict, target2num, num_entities, cov_mod='3', seqid_mod='1', num_mod='2'):
    ...
    for t, pairs in query_dict.items():
        ...
        for (q_id, t_id ), values in pairs.items():
            ...
            if seqid_mod == '1':
                # query seqid
                weight = values['qtmscore']
            elif seqid_mod == '2':
                # target seqid
                weight = values['ttmscore']
            elif seqid_mod == '3':
                # min seqid
                weight = min(values['qtmscore'], values['ttmscore'])
            elif seqid_mod == '4':
                # max seqid
                weight = max(values['qtmscore'], values['ttmscore'])
            else:
                # default is no seqid
                weight = 1
...
def main(args):
    ...
    pdb_files = [f for f in os.listdir(args.inputDir) if f.endswith('.pdb')]
    ...
    filename = 'foldseek-e'+str(args.evalue)+'.tsv'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--foldseekPath", type=str, default=shutil.which('foldseek'), help="Path to the FoldSeek executable.")
